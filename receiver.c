...
int main() {
    ...
    char buffer_auth[M_Authentication_SIZE]="0101 0001 1100 0000";
    ...
    char namealgo[250]; // to the name of the cc algorithm
    socklen_t length;
    ...
    messageexit[0]= '0'; // create the message exit that the receiver will send if the user want to exit.
    int sockfd = -1;
    int countbytes = 0;
        ...
        while (1) {
            ...
            if (countbytes == HALF_SIZEFILE) {
                ...
                if (checkPart == 0) { //To handle with  the first recv part
                    ...
                    send(client_socket,buffer_auth, sizeof(M_Authentication_SIZE),0);
                    ...
                    *(part1 + c1) = time_spent;
                    ...
                    //gettimeofday(&end, 0);
                    bzero(namealgo, sizeof(namealgo)); // update
                    printf("\nSwitching to Reno algorithm\n");
                    strcpy(namealgo, "reno"); // put the name of the cc algorithm inside the name algo
                ...
                part1 = (double *) realloc(part1, sizeof(double) * (c1 + 1));
                part2 = (double *) realloc(part2, sizeof(double) * (c2 + 1));
                ...
                if (size2 == SIZEFILE) { // if we go inside then we rcv all the file
                    ...
                    if (User_Decision_MESSAGE[0] == messageexit[0]) { // if it's equal to exit-message then stop
    ...
    printTimes(part1, part2, c1, sum_for_average_cubic, sum_for_average_reno);
